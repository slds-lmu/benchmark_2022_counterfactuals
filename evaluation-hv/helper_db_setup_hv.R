read_in_cfactual_obj = function(this_job, reg) {
  ...
  if (is_keras) {
    ...
    model_dir = file.path("models/prod/keras/neural_network")
...
add_results_to_db = function(data_set_name) {
  ...
  template_names = c(
    names(readRDS("data/data_storage/data_list.RDS")[[data_set_name]]$data),
  ...
  for (job_id in jobs_of_this_data_set$job.id) {
    ...
    cfactuals_obj = try(read_in_cfactual_obj(this_job, reg))
